...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            path = "C:/Users/Alston Soares/Desktop/Demo-Codlions/website/static/profile_pics/"
            ...
            new_user = User(unique = uniqueUserCode, filetype = filetype, image_file = result, image_data=image_file.read(), image_mimetype=image_mimetype, email=email, username=username, password=generate_password_hash(pass1, method='sha256'))
            ...
            if new_user.id != 1:
                ...
                db.session.add(new_user)
                db.session.commit()
                parent_dir = "C:/Users/Alston Soares/Desktop/Demo-Codlions/website/static/storage/"
                dir = new_user.username
                my_path = os.path.join(parent_dir, dir)
                os.mkdir(my_path)
                return redirect('/')
            else:
